...
| TD-013 | ~~`executeToolRequestSimulated()` is the only Execution Adapter~~ — narrowed Goal 16: Twilio (`send_sms`) now has a real production adapter (`apps/api/src/services/providerAdapters/twilioSmsAdapter.ts`) with retry and circuit breaker; all other 18 registered providers still use the simulated fallback. Generic registry-driven dispatch is blocked on provider/tool registry entries lacking endpoint/HTTP-method metadata | Goal 8 | High | unassigned | narrowed |
| TD-014 | ~~`CredentialReference.secretRef` is a schema for a pointer only~~ — narrowed Goal 16: `CredentialResolver` now reads `process.env[secretRef]` as an **explicit placeholder** (documented as such in ADR 0015); still no real external secret-store integration (Vault, AWS Secrets Manager), encryption at rest, rotation, or versioning — deferred to Goal 17 | Goal 8 | High | unassigned | narrowed |
