...
function workflowStatusColor(status: string): "blue" | "green" | "red" | "yellow" | "neutral" {
        ...
        {FILTERS.map((f) => {
          ...
          return (
            ...
            >
              {f.label}
              {count > 0 && (
                <span className="ml-1.5 rounded-full bg-elevated px-1.5 py-0.5 text-xs text-text-muted">{count}</span>
              )}
            </button>
          );
        })}
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-text-primary truncate">{ex.workflowName || ex.workflowKey}</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {ex.status === "failed" && (
                      <button
                        ...
                        className="rounded bg-elevated px-2 py-0.5 text-xs text-text-secondary hover:bg-border disabled:opacity-50 transition-colors"
                      >
                              ...
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  {step.durationMs !== null && (
                                    <span className="text-xs text-text-muted">{fmtDuration(step.durationMs)}</span>
                                  )}
                                  <span className="text-xs text-text-muted capitalize">{step.taskType}</span>
                                </div>
                              </div>
                              {step.outputSummary && (
                                <p className="mt-0.5 text-xs text-text-muted truncate">{step.outputSummary}</p>
                              )}
                              {step.errorMessage && (
