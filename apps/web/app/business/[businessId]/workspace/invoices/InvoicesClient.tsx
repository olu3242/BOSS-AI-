    ...
    <div className="flex flex-col gap-8">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60" onClick={() => setShowForm(false)}>
          <div
            className="h-full w-full max-w-lg overflow-y-auto bg-neutral-950 p-6 shadow-xl border-l border-neutral-800"
            ...
            <form onSubmit={handleCreate} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="grid grid-cols-12 gap-1 text-[10px] text-neutral-600 px-1">
                  ...
                  {lineItems.map((li, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-1">
                      <input
                        ...
                        className="col-span-5 rounded border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-xs text-white placeholder:text-neutral-600 focus:border-neutral-500 focus:outline-none"
                      />
                      <input
                        type="number" min="1" value={li.quantity}
                        onChange={(e) => updateLineItem(idx, "quantity", parseInt(e.target.value) || 1)}
                        className="col-span-2 rounded border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-xs text-white text-right focus:border-neutral-500 focus:outline-none"
                      />
                      <input
                        type="number" min="0" step="1" value={li.unitPriceCents}
                        onChange={(e) => updateLineItem(idx, "unitPriceCents", parseInt(e.target.value) || 0)}
                        placeholder="Cents"
                        className="col-span-3 rounded border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-xs text-white text-right focus:border-neutral-500 focus:outline-none"
                      />
                      <button
                        type="button" onClick={() => removeLineItem(idx)}
                        className="col-span-1 flex items-center justify-center text-neutral-600 hover:text-red-500 transition-colors"
                      >
                        ✕
                      </button>
                    </div>
                ...
                <div className="mt-3 flex flex-col items-end gap-1 text-sm">
                  <div className="flex items-center gap-4">
                    ...
                    <input
                      ...
                      className="w-20 rounded border border-neutral-700 bg-neutral-900 px-2 py-1 text-xs text-white text-right focus:border-neutral-500 focus:outline-none"
                    />
                  </div>
                  <p className="text-neutral-500 text-xs">Subtotal: {formatMoney(subtotal)}</p>
                  <p className="text-neutral-500 text-xs">Tax: {formatMoney(tax)}</p>
                  <p className="font-medium text-white">Total: {formatMoney(total)}</p>
                </div>
              </div>
            ...
            <div key={inv.id} className="flex items-center gap-4 px-5 py-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-mono text-sm text-neutral-300">{inv.invoiceNumber}</p>
                  <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-medium capitalize ${STATUS_STYLE[inv.status] ?? "bg-neutral-800 text-neutral-400"}`}>
                    {inv.status}
                  </span>
                </div>
              ...
              <div className="shrink-0 text-right">
                <p className="font-medium text-white">{formatMoney(inv.totalCents, inv.currency)}</p>
                <p className="text-[11px] text-neutral-600">{inv.currency}</p>
              </div>
