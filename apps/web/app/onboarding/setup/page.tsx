...
function Step4({ data, update }: { data: WizardData; update: (k: keyof WizardData, v: string) => void }) {
  return (
    <>
      ...
      <p className="ob-subtitle">List the products or services you sell. BOSS uses these to generate marketing copy, track revenue by service, and identify upsell opportunities.</p>
...
function Step5({
  ...
  return (
    <>
      ...
      <p className="ob-subtitle">Select the tools you currently use. BOSS will connect to them or help you migrate away from what's slowing you down.</p>
      ...
      <p style={{ fontSize: "0.75rem", color: "#6b6b76" }}>
        {data.existingTools.length === 0
          ? "Select any that apply — skip if you're starting fresh"
          : `${data.existingTools.length} selected`}
      </p>
    </>
  );
}

// ── Step 6: First AI setup ────────────────────────────
...
function Step6({
  ...
  return (
    <>
      ...
      <p className="ob-subtitle">Choose which AI employees to deploy first. You can add or remove them at any time from your workspace.</p>
      ...
      <p style={{ fontSize: "0.75rem", color: "#6b6b76" }}>
        {data.aiAgents.length === 0
          ? "Select at least one AI employee to activate"
          : `${data.aiAgents.length} AI employee${data.aiAgents.length === 1 ? "" : "s"} selected`}
      </p>
    </>
  );
}
